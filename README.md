...
通过 MCP 风格的 HTTP API，让 AI Agent 远程控制云端运行时（Google Colab / ModelScope），执行 Python 代码、利用 GPU 资源、进行数据分析。
...
├── app.py                         # 服务器主程序 v2.0 (ModelScope 用)
...
| 限制 | Google Colab | ModelScope | 本地/Codespace |
|------|-------------|------------|-----|
| 会话时长 | ~90 分钟无活动 | 长期运行 | 长期运行 |
| 内存 | ~12 GB RAM | ~16 GB RAM | 取决于硬件 |
| GPU | Tesla T4/K80 | xGPU（申请） | 取决于硬件 |
| 公网 URL | AiTun 免注册 | 固定 URL | AiTun 免注册 |
